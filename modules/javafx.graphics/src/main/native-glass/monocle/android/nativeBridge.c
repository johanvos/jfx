...
void initializeFromJava (JNIEnv *env) {
    ...
    GLASS_LOG_FINEST("Initializing native Android Bridge got1");
...
fprintf(stderr, "androidSetNativeWindow asked now, set to %p\n", nativeWindow);
    initializeFromNative();
    androidWindow = nativeWindow;
fprintf(stderr, "afer androidSetNativeWindow asked now, set is %p\n", nativeWindow);
}
...
fprintf (stderr, "first init from java...\n");
    initializeFromJava(env);
fprintf (stderr, "done init from java, window = %p\n", androidWindow);
