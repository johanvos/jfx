...
class GraphicalCellStub extends IndexedCellShim<Node> {
    ...
    public void updateItem(Node item, boolean empty) {
        System.err.println("[GC] updateItem called on " +this+" with item = "+item+" with height = "+this.getHeight());
        Thread.dumpStack();
myItem = item;
        ...
        if (empty || item == null) {
            ...
            setGraphic(item);
        System.err.println("[GC] updateItem DONE with item = "+item+" with height = "+this.getLayoutBounds().getHeight()+
                " and cell itself h = "+item.getLayoutBounds().getHeight());
        }
    }

    @Override
    public void updateIndex(int i) {
        System.err.println("[GC] UPDATEINDEX asked with i = "+i);
        super.updateIndex(i);
    ...
    protected double computePrefHeight(double width) {
        ...
        System.err.println("[JV] GC, computePrefHeight, w = " + width + ", answer = " + answer);
        if ((idx > -1) && (idx < circleList.size())){
            answer = 2* circleList.get(idx).getRadius()+ 6;
        }
        System.err.println("[JV] GC, computePrefHeight, w = " + width + ", answer2 = " + answer);
